...
const getPrivateFilms = async (pageNumber) => {
  ...
  var path = SERVER_URL + 'films/private';
 ...
 */
 const getPublicFilmsToReview = async (pageNumber) => {
  // film.watchDate could be null or a string in the format YYYY-MM-DD
  var path = SERVER_URL + 'films/public/invited';
  if(pageNumber != undefined) path += '?pageNo=' + pageNumber;
  return getJson(fetch(path, { credentials: 'include' })).then( json => { 
    localStorage.setItem('totalPages',  json.totalPages);
    localStorage.setItem('currentPage', json.currentPage);
 ...
 */
 const getFilmReviews = async (filmId, pageNumber) => {
  // film.watchDate could be null or a string in the format YYYY-MM-DD
  var path = SERVER_URL + 'films/public/' + filmId + '/reviews';
  if(pageNumber != undefined) path += '?pageNo=' + pageNumber;
  return getJson(fetch(path, { credentials: 'include' })).then( json => { 
    localStorage.setItem('totalPages',  json.totalPages);
    localStorage.setItem('currentPage', json.currentPage);
...
const getFilm = async (filmId) => {
  return getJson( fetch(SERVER_URL + 'films/' + localStorage.getItem('filmsType') + '/'+ filmId, { credentials: 'include' }))
...
async function updateFilm(film) {
  ...
  const response = await fetch(
      SERVER_URL + "films/"+ localStorage.getItem('filmsType')  + "/" + film.id, {
      method: 'PUT',
      headers: {
...
async function deleteFilm(film) {
  const response = await fetch(SERVER_URL + "films/" + localStorage.getItem('filmsType')  + "/" + film.id, {
    method: 'DELETE',
    credentials: 'include'
  });
  if(!response.ok){
 ...
 */
 function issueReview(filmId, user) {
  const jsonUser = JSON.stringify([{filmId: filmId, reviewerId : user.userId}]);
  return getJson(
    fetch(SERVER_URL + "films/public/" + filmId + "/reviews", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
 ...
 */
 const getReview = async (filmId, reviewerId) => {
  return getJson( fetch(SERVER_URL + 'films/public/' + filmId + "/reviews/" + reviewerId, { credentials: 'include' }))
...
const logIn = async (credentials) => {
  return getJson(fetch(SERVER_URL + 'users/authenticator?type=login', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
const logOut = async() => {
  ...
  return getJson(fetch(SERVER_URL + 'users/authenticator?type=logout', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
